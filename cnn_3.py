...
def create_enhanced_cnn_model(lidar_len: int, scalar_len: int,
                               conv_channels=(32, 64, 128),
                               dense_units=(128, 64), dropout=0.1):
    """
    Enhanced two-input CNN with multi-scale processing and attention.
    Inputs:
    ...
    # --- Enhanced LiDAR branch with multi-scale processing ---
    ...
    # Multi-scale parallel processing for first layer
    scales = []
    kernel_sizes = [3, 7, 15]  # local, medium, global patterns
    for k in kernel_sizes:
        branch = layers.Conv1D(conv_channels[0]//3, kernel_size=k, padding="same")(x)
        branch = layers.BatchNormalization()(branch)
        branch = layers.ReLU()(branch)
        scales.append(branch)
    
    x = layers.Concatenate()(scales)  # Combine multi-scale features
    
    # Continue with remaining conv layers with residual connections
    for i, ch in enumerate(conv_channels[1:], 1):
        shortcut = x
        ...
        if shortcut.shape[-1] == ch and shortcut.shape[1] == x.shape[1]:
            x = layers.Add()([x, shortcut])
    
    # Attention mechanism before pooling
    attention_weights = layers.Conv1D(1, kernel_size=1, activation='sigmoid', name="lidar_attention")(x)
    ...
    # Add small MLP for scalar features
    s = layers.Dense(32, activation="relu")(s)
    s = layers.Dropout(dropout * 0.5)(s)  # Light dropout on scalar features
    s = layers.Dense(16, activation="relu")(s)

    # --- Enhanced Fusion + head ---
    z = layers.Concatenate()([x, s])
    for i, u in enumerate(dense_units):
        z = layers.Dense(u, activation="relu", 
                        kernel_regularizer=keras.regularizers.l2(1e-4))(z)
        # Increasing dropout in deeper layers
        z = layers.Dropout(dropout + i * 0.05)(z)
        
    out = layers.Dense(2, name="cmd_out")(z)           # -> [v_hat, w_hat]

    model = keras.Model([lidar_in, scal_in], out, name="EnhancedLidarCNNPolicy")

    # Use fixed learning rate for compatibility with ReduceLROnPlateau
    model.compile(
...
def large_dataset(input_directory, single_dkr_flag, adaptive_flag, enhanced=True):
    """
    ...
    new_dir = os.path.join("data_set", f"{timestamp}_{model_type}")
    os.makedirs(new_dir, exist_ok=True)

    yaml_data = {
        "run": {"timestamp": timestamp, "output_dir": os.path.abspath(new_dir), "model_type": model_type},
        "datasets": [],
        "combined": {},
    }
    ...
    # Build, Adapt, Train
    # =========================
    epochsVal = 200  # Reduced from 500 since enhanced model converges faster
    early_stopping = keras.callbacks.EarlyStopping(
        monitor='val_loss', mode='min', patience=15, min_delta=0.001, restore_best_weights=True
    )
    ...
    # Choose model based on enhanced flag
    if enhanced:
        model = create_enhanced_cnn_model(lidar_len=L, scalar_len=S, dropout=0.15)
        print("[Model] Using enhanced CNN with multi-scale processing and attention")
    else:
        # Fallback to original model (you'd need to import the original function)
        model = create_cnn_model_with_norm(lidar_len=L, scalar_len=S)
        print("[Model] Using basic CNN")
    ...
    # Data augmentation callback (add noise during training)
    class NoiseAugmentation(keras.callbacks.Callback):
        def __init__(self, noise_std=0.02):
            self.noise_std = noise_std
        
        def on_batch_begin(self, batch, logs=None):
            # This is a simplified version - in practice you'd modify the data generator
            pass
    ...
    history = model.fit(
        ...
        epochs=epochsVal, batch_size=128, callbacks=callbacks, verbose=1  # Smaller batch size for enhanced model
    )

    # =========================
    # Save Keras + ONNX + Graphs
    # =========================
    model_path = os.path.join(new_dir, f"{timestamp}_{model_type}.keras")
    model.save(model_path)
    print(f"[Keras] Wrote: {model_path}")

    onnx_path = os.path.join(new_dir, f"{timestamp}_{model_type}.onnx")
    ...
    print(f"\n[Training Complete] Best val_loss: {final_val_loss:.4f}, Best val_mae: {final_val_mae:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced Robot CNN (LiDAR+Scalars) Training")
    ...
    parser.add_argument("--basic", action='store_true',
                        help="Use basic CNN architecture instead of enhanced version")
    args = parser.parse_args()

    # Use enhanced model by default, unless --basic is specified
    enhanced = not args.basic

    # Use the same behavior you showed: only the 'large' pipeline here
    if args.large:
        large_dataset(args.input_bag, args.single_dkr, args.adaptive, enhanced)
    else:
